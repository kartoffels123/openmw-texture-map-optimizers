...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python land_texture_scanner.py <esp_file> [esp_file2 ...]")
        print("       python land_texture_scanner.py --dir <directory>")
        print("       python land_texture_scanner.py --mods <mods_directory>")
        sys.exit(1)

    if sys.argv[1] == "--mods":
        # Scan a mods directory structure (MO2 style)
        mods_dir = Path(sys.argv[2])
        print(f"Scanning mods directory: {mods_dir}")
        print("Looking for ESP/ESM files in mod subfolders...\n")

        land_textures = scan_mods_directory_for_land_textures(mods_dir, verbose=True)
        ...
        all_textures = scanner.get_land_texture_paths(plugins)
        print(f"\n=== Summary ===")
        print(f"Total unique land textures: {len(all_textures)}")

    else:
        # Scan specific plugin files
        plugins = [Path(p) for p in sys.argv[1:]]
        scanner = LandTextureScanner()

        print(f"Scanning {len(plugins)} plugin(s)...")
